...
class CustomerPaymentListPage extends StatelessWidget {
  final int customer;
  const CustomerPaymentListPage({super.key, required this.customer});
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => InventoryBloc(InventoryServices(
        ...
        ..add(
          GetPaymentsForCustomer(page: 1, customer: customer),
        ),
      child: Builder(builder: (context) {
        return CustomerPaymentListPageChild(
          customer: customer,
        );
      }),
    );
...
class CustomerPaymentListPageChild extends StatefulWidget {
  final int customer;
  const CustomerPaymentListPageChild({super.key, required this.customer});

  @override
  State<CustomerPaymentListPageChild> createState() =>
      _CustomerPaymentListPageChildState();
}

class _CustomerPaymentListPageChildState
    extends State<CustomerPaymentListPageChild> {
  int currentPage = 1;
  ...
  void runBloc() {
    context.read<InventoryBloc>().add(
          GetPaymentsForCustomer(page: currentPage, customer: widget.customer),
        );
  }
